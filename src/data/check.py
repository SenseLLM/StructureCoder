...
def _select(data):
    ...
    for solution in data['processed_solutions']:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--in_path', default="source/apps_train_format.jsonl", type=str)
    parser.add_argument('-o', '--out_path', default="source/apps_train_format_check.jsonl", type=str)
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor, open(args.out_path, "w") as fw:
        for batch_data in [data[i:i + 1000] for i in range(0, len(data), 1000)]:
            futures = {executor.submit(_select, d): i for i, d in enumerate(batch_data)}
