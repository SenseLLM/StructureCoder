...
def run_test(input_output, test=None, debug=False):
    """
...
def _temp_run(input_output, solution, results):

    try:
        result = run_test(input_output, solution, debug=False)
    except Exception as e:
        results.append(-2)
    
    try:
        result = min(int(np.all(r) if isinstance(r, np.ndarray) else r) for r in result)
        results.append(result)
    except Exception as e:
        results.append(-2)

def _check_correctness(input_output, solution):
    ...
    p = multiprocessing.Process(
        target=_temp_run,
        args=(input_output, solution, result),
    )
    p.start()
    p.join(timeout=20)
...
def _check(data):
    ...
    for output in data['output_post']:
        solution = data['pre'] + output + data['suf']
        result = _check_correctness(data['input_output'], solution)
        data['result_post'].append(result)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--in_path', default="/mnt/cache/code/train/dpo_coder/data/source/apps_train_format.jsonl", type=str)
    parser.add_argument('-o', '--out_path', default="/mnt/cache/code/train/dpo_coder/data/source/apps_train_format_check.jsonl", type=str)

    args = parser.parse_args()
    
    data = load_jsonl(args.in_path)

    start, total, i = time.time(), len(data), 0

    with concurrent.futures.ProcessPoolExecutor() as executor, open(args.out_path, "w") as fw:
        for batch_data in [data[i:i + 1000] for i in range(0, len(data), 1000)]:
            futures = {executor.submit(_check, d): i for i, d in enumerate(batch_data)}
            for future in concurrent.futures.as_completed(futures):
                ...
                if i % 100 == 0:
                    ...
                    print(f"{i}/{total} processed, {t:.2f}/{all_time:.2f} ({rest_time:.2f}) sec", flush=True)
